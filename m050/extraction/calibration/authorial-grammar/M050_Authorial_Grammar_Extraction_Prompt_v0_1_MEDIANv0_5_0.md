# Authorial Grammar source-bounded extraction prompt v0.1
...
Convert the supplied Authorial Grammar source blocks into grounded, atomic authorial-rule proposals. Use only the text inside `SOURCE_BLOCKS`. Do not use background knowledge, prior atoms, other MEDIAN sources, or inferred cross-source authority.
...
For every supplied eligible or review-required block, return exactly one disposition. Split independent rules into separate atoms. Keep dependent qualifications with the rule they qualify. Copy `exact_source_text` exactly and contiguously from the named block. Never silently repair or paraphrase the evidence span.
