#!/usr/bin/env python3
"""Verify the clean-contract, halted-run requantized extraction-machine checkpoint."""
...
PRIOR_PATH = REPO_ROOT / "m050/tools/m050_guard_v0_8.py"
AGENTS = REPO_ROOT / "AGENTS.md"
AGENTS_OVERRIDE = REPO_ROOT / "AGENTS.override.md"
ACTIVE_INDEX = REPO_ROOT / "m050/extraction/control/M050_Active_Control_Index_v0_14_MEDIANv0_5_0.json"
PREDECESSOR_INDEX = REPO_ROOT / "m050/extraction/control/M050_Active_Control_Index_v0_13_MEDIANv0_5_0.json"
CHECKPOINT = REPO_ROOT / "m050/extraction/control/M050_Current_State_Checkpoint_v0_5_MEDIANv0_5_0.json"
PREDECESSOR_CHECKPOINT = REPO_ROOT / "m050/extraction/control/M050_Current_State_Checkpoint_v0_4_MEDIANv0_5_0.json"
CHECKPOINT_REPORT = REPO_ROOT / "m050/extraction/control/M050_Current_State_Checkpoint_v0_5_MEDIANv0_5_0.md"
BOOTSTRAP = REPO_ROOT / "m050/extraction/control/M050_New_Task_Bootstrap_v0_4_MEDIANv0_5_0.md"
...
SPEND = REPO_ROOT / "m050/extraction/audit/spend-envelopes/M050_Provider_Spend_Envelope_001_After_C0001_v0_1_MEDIANv0_5_0.json"
...
def validate_root_contract(errors: list[str]) -> None:
    ...
    required = (
        "M050_Active_Control_Index_v0_14_MEDIANv0_5_0.json",
        "M050_Current_State_Checkpoint_v0_5_MEDIANv0_5_0.md",
        "M050_New_Task_Bootstrap_v0_4_MEDIANv0_5_0.md",
        "m050_guard_v0_9.py",
        "24 / 22 / 4 / 18 = 14 + 4",
...
def validate_active_index(errors: list[str]) -> dict:
    ...
    if index.get("schema_version") != "M050-ACTIVE-CONTROL-INDEX-0.14":
    ...
    if index.get("execution_state") != "AUTHORIAL_GRAMMAR_SOURCE_RUN_HALTED_REQUANTIZED_PILOT_FROZEN":
    ...
    if (
        ...
        or calibration.get("current_state") != "requantized_pilot_frozen_waiting_exact_authorization"
        or calibration.get("pilot_acceptance_releases_full_source") is not False
        or calibration.get("prior_source_run_revoked") is not True
    ...
    required_machine = {
        ...
        "reconciled_remaining_usd": "1.918382",
...
def validate_machine(errors: list[str]) -> None:
    ...
    for path in (STANDARD, CONTROLLER, ENGINE_MODULE, ENGINE_TEST, COMPATIBILITY, PLAN, FREEZE, HALT, SPEND):
        if not path.is_file():
    ...
    if (
        ...
        or compatibility.get("offline_verification", {}).get("offline_tests_passed") != EXPECTED_TESTS
    ...
    if (
        ...
        or freeze.get("authority", {}).get("provider_call_authorized") is not False
        or freeze.get("binding", {}).get("pilot_chunk_id") != "C0001"
    ):
        errors.append("quantized C0001 pilot freeze drifted")
    ...
    if spend.get("active") is not False or spend.get("remaining_usd") != "1.918382":
...
def validate_checkpoint(errors: list[str]) -> None:
    ...
    if checkpoint.get("status") != "AUTHORIAL_GRAMMAR_SOURCE_RUN_HALTED_REQUANTIZED_PILOT_FROZEN":
...
def main() -> int:
    ...
    engine_files, engine_digest = PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.engine_snapshot()
    if args.with_tests and PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.run_tests():
    ...
    print("- next source: Authorial Grammar; C0001 run halted and release revoked")
    print("- exact spend: $0.081618; inactive envelope remaining: $1.918382")
    print("- provider calls, pilot release, full-source release, and spend reactivation: not authorized")
    print("- machine: one declarative controller; scaffold + complete-source replan; sequential call/review")
    print("- quantization: 20 target blocks; chunk count generated as 12, never a manifest input")
