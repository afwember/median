# Authorial Grammar source-bounded extraction prompt v0.3
...
## Lean structural evidence rule

A Markdown table's header row and delimiter row are structural layout metadata, not authorial-rule evidence. Return `no_substantive_claim` with an empty `atoms` array for both rows. Never create an atom stating that a table has named columns or describing its layout. Substantive table body rows remain eligible and must be interpreted with their lead-in and header context.

This rule is generic: apply it to every table, not only to a named block or example.
...
Use `review_required` rather than guessing when ownership, status, scope, example polarity, table body meaning, or atomic boundaries remain uncertain. Do not create identifiers, statuses, definitions, or authorities absent from the supplied block.
