...
## Game Design Document v0.4.3 — Art Direction Edition
...
> **About this version.** v0.4.3 is a surgical edition. It changes no structure and adds no systems. It folds in eight decisions arising from the Phase 2 art production pass — chiefly **the anthropomorphism line**, which was the longest-standing open item in the document — and closes four Appendix D entries. It is governed by a settled record of **93 design decisions**. A complete change record appears in Appendix C; open items are collected in Appendix D.
>
> *v0.4.2, the preceding edition, was the restructuring pass: eight parts, the encounter and exposure model, and the mode architecture.*
...
Two recurring ideas are deliberately *not* pillars. The sound wall's status as a real, non-arbitrary world boundary is a single grounding fact, stated in Section 7. And "real ecology as inspiration, relaxed wherever it fights good gameplay" is a loosely-binding working principle — one with a single organizing idea behind it, the **Edge Effect** (Section 9.3), which explains an existing pattern rather than adding a mechanic.
...
The pattern is visible in what has been *refused*: Reputation, Knowledge, and Visitors-Helped were each proposed as tracked stats and each rejected, because existing systems already carry those stories. The Records introduce no currency. A proposed post-expedition sharing bonus was stripped to pure information. A construction Speed-Up was cut outright rather than reskinned.
...
**There is no cross-recruitment among the core species at any point.** A rabbit colony never recruits squirrels or mice, and so on. The "which species do you want to play" choice stays meaningful for the entire run, and the found-family beat lives entirely in the Guest system (Section 20).

Species is not a chapter a reader can skip. It flavors the hub name, every structure family's vocabulary, the Given Name banks, the colony-naming imagery, the spatial signature system, and the way a party behaves in the field. It is stated here, early, because everything after this point is coloured by it.
...
**"Population-per-Upkeep" is not a separate stat.** A species' expendability is already expressed as a function of its Food Consumption ratio. No new tracked stats were added in v0.4.1 or v0.4.2.
...
The mouse's expedition identity is deliberately *not* a divided-attention macro-mechanic. The rejected "pheromone macro-swarm" concept is not canonical.
...
Alternatives considered and rejected on record for Roaring Iron: *Beasts* (an animal does not find "beast" pejorative — it fails to convey otherness at all), *Growler* (unavoidable beer connotation), *Shells* (too passive for something that kills by momentum), and *Dragons* (an excellent image-match — headlights as fire-eyes, exhaust as smoke — but a borrowed high-fantasy creature noun, failing the same test as borrowing a *Watership Down* name; keep the imagery, drop the genre word).

Two further terms name the campaign's founding catastrophe and the home it took:

- **The Founding Escape** — the event itself. The designer's plain term is also the in-world one, which suits a folk register deliberately tuned homely and plain rather than ornate (Section 22.2). It is categorically *not* an instance of the Mowing: the Mowing is cyclical, evadable and survivable, while the Founding Escape is a singular permanent apocalypse (Section 35.1). Considered and not adopted as alternatives: *The Widening*, *The Unmaking*, *The Breaking*, *The Uprooting*, *The Flattening*, *The Overturning*.
...
The Home Median is one segment in a longitudinal chain of medians extending up and down the corridor. Each remote segment is a **Median Reach** (the term "Median Zone" is retired). Reaches differ in width, hydrology, vegetation, infrastructure, exposure, predators, lane count, and resource character.
...
**Each Reach is unified, and adjacent Reaches may differ sharply.** A Reach is internally coherent — one place with one character — and is deliberately *not* sub-divided into internal micro-zones. Internal variation is already supplied by Anchor Points (Section 8.2), which guarantee a mix rather than a monoculture; a second subdivision layer would be redundant.
...
Chemical Runoff and Invasive Plants, both considered, are **not** adopted as separate named hazards — their flavor is absorbed into the Oil Leak manifestation and the Margin's general texture.
...
**Nesting Season** is the named breeding window that earlier drafts gestured at without naming. It is the only time a **Hearth** can bring a new named citizen into the world, and it remains rare, expensive, and player-authorized (Section 17.3).
...
**Road Work reshapes the event pool.** While the persistence window runs, Road Work acts as a **card-pool modifier** on Choice-Event Cards: it tilts the pool toward hazard and disruption cards and raises event frequency. This is how Road Work is made "loud" — through an existing system rather than a new one — and it is deliberately *not* implemented as a calendar season.
...
- **No numeric countdowns anywhere.** The non-numeric telegraph rule, originally scoped to Road Work, generalizes to the entire event system. Every telegraph is qualitative and diegetic — a smell, a sound, a changed sky, a Guest's unease. Nothing anywhere in MEDIAN tells the player "3 days until X."
...
- **There is no Speed-Up mechanic of any kind.** Cut outright rather than guarded against, which removes the premium-currency-adjacent design pressure entirely.
...
Every citizen has a **Character Record** (Section 21.2), presented in two views — a **Likeness** for facts and a **Tale** for history. The term "dossier" is retired as too clinical for a game about knowing everyone by name.
...
**The game must never imply that disability automatically grants genius or supernatural compensation.** Veteran strengths emerge from lived history and role design. The old v0.1 line pairing every maiming with "a small compensating trait" is deliberately not pursued.
...
**Trigger.** A citizen goes tharn when their **exposure exceeds their stress threshold** during an encounter (Section 25.5). This is a single unified trigger built on machinery that exists only inside an encounter, with per-species flavor in how the freeze *manifests* rather than in what causes it. Earlier proposals for separate load-based and isolation-based triggers are both absorbed: load already raises exposure, and a citizen whose exposure has spiked far above the party's is, definitionally, the one who is out there alone.
...
**The slot is called Tool and not "Weapon," and the distinction is deliberate rather than cosmetic.** A slot named Weapon would quietly reintroduce the light-weaponization drift this design has repeatedly rejected. Animals in MEDIAN use objects; they do not arm themselves.
...
The **Deer Mouse** is removed from the roster — redundant with the core wood-mouse species and too narrow a niche.
...
There is deliberately **no sharing bonus.** An earlier proposal attached a colony-growth buff to debriefing; it is removed. Map and preview updates are simply "you now know what you saw" — not a reward to be farmed. The debrief is narrative and informational only.
...
*Flagged for dedicated design:* Crossing is the game's signature mechanic and the least specified system in this document. How it actually plays moment to moment — input model, failure states, how the party moves as a group, how longitudinal travel differs (Section 7.3) — needs its own pass (Appendix D).
...
**An encounter is not automatically a fight.** Combat is only one response; others include display, evasion, bribery, negotiation, trickery, surrendering part of the haul, rescue, and withdrawal. In earlier drafts these were outcomes the fiction explained after the fact. They are now **player choices made before the first round** — a promotion of existing canon rather than a new mechanic.
...
**Rejected: a low-probability roll for extra length.** Length is the one place randomness feels least earned. A free extra round is a gift; a costly one is arbitrary. Closeness is the honest trigger.
...
*(This tier was formerly "Fortified Warren." **Warren, Den, and Burrow are species terminology** — rabbit, squirrel, and mouse respectively — and are never used generically. A tier name shared by all three campaigns cannot borrow one of them.)*
...
The term **"dossier" is retired** throughout the interface and this document. It reads as a file on a stranger, and the whole premise is that these are not strangers.
...
Two criteria specific to this document's systems: players should **read the Chronicle voluntarily** rather than treating it as a menu, and should **quote the Laws of the Median back at the game** — using folklore as shorthand for mechanics, which is the surest sign the tutorialization-as-culture approach landed.
...
*End of MEDIAN Game Design Document v0.4.3 — Art Direction Edition. Values marked as placeholders in Appendix D remain open by design.*
