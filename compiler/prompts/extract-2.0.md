...
**`weight`** — `STATE` the finished GDD states this as fact about the world ·
`SHOW` the game must show it, whether or not it is stated · `SAY` in-world
voice · `SILENT` internal only, never reaches the reader.
...
emit:
    ...
    {"loc":"4.2¶3",
     ...
     "type":"REQ","weight":"STATE","status":"canonical","owner":"home.dwell.roles"}
