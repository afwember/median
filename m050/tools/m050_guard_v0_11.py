#!/usr/bin/env python3
"""Verify the clean-contract, halted C0002 semantic-context extraction-machine checkpoint."""
...
PRIOR_PATH = REPO_ROOT / "m050/tools/m050_guard_v0_10.py"
AGENTS = REPO_ROOT / "AGENTS.md"
AGENTS_OVERRIDE = REPO_ROOT / "AGENTS.override.md"
ACTIVE_INDEX = REPO_ROOT / "m050/extraction/control/M050_Active_Control_Index_v0_16_MEDIANv0_5_0.json"
PREDECESSOR_INDEX = REPO_ROOT / "m050/extraction/control/M050_Active_Control_Index_v0_15_MEDIANv0_5_0.json"
CHECKPOINT = REPO_ROOT / "m050/extraction/control/M050_Current_State_Checkpoint_v0_7_MEDIANv0_5_0.json"
PREDECESSOR_CHECKPOINT = REPO_ROOT / "m050/extraction/control/M050_Current_State_Checkpoint_v0_6_MEDIANv0_5_0.json"
CHECKPOINT_REPORT = REPO_ROOT / "m050/extraction/control/M050_Current_State_Checkpoint_v0_7_MEDIANv0_5_0.md"
BOOTSTRAP = REPO_ROOT / "m050/extraction/control/M050_New_Task_Bootstrap_v0_6_MEDIANv0_5_0.md"
...
def validate_root_contract(errors: list[str]) -> None:
    ...
    required = (
        "M050_Active_Control_Index_v0_16_MEDIANv0_5_0.json",
        "M050_Current_State_Checkpoint_v0_7_MEDIANv0_5_0.md",
        "M050_New_Task_Bootstrap_v0_6_MEDIANv0_5_0.md",
        "m050_guard_v0_11.py",
        "24 / 22 / 4 / 18 = 14 + 4",
...
def validate_active_index(errors: list[str]) -> dict:
    ...
    if index.get("schema_version") != "M050-ACTIVE-CONTROL-INDEX-0.16":
    ...
    if index.get("execution_state") != "AUTHORIAL_GRAMMAR_QUANTIZED_SOURCE_RUN_HALTED_C0002_CONTEXT_SPLIT":
    ...
    if (
        ...
        or calibration.get("current_state") != "source_run_halted_waiting_author_calibration_direction"
        or calibration.get("accepted_quantized_pilot_preserved") is not True
        or calibration.get("remaining_source_run_revoked") is not True
        or calibration.get("failure_kind") != "semantic_context_split"
        or calibration.get("additional_provider_calls_authorized") != 0
        or calibration.get("replan_authorized") is not False
    ):
    ...
    required_machine = {
        ...
        "semantic_list_leadin_preservation_defect_open": True,
        "claude_cache_ttl": "1h",
...
def validate_machine(errors: list[str]) -> None:
    ...
    if (
        halt.get("release_revoked") is not True
        or halt.get("additional_provider_calls_authorized") != 0
        or halt.get("failed_chunk_id") != "C0002"
        or halt.get("defect", {}).get("kind") != "semantic_context_split"
    ):
...
def validate_bootstrap(errors: list[str]) -> None:
    ...
    required = (
        ...
        "Exact corpus vector",
        "C0002 semantic list-boundary defect",
        "chunk count is a generated result",
...
def validate_checkpoint(errors: list[str]) -> None:
    ...
    if checkpoint.get("status") != "AUTHORIAL_GRAMMAR_QUANTIZED_SOURCE_RUN_HALTED_C0002_CONTEXT_SPLIT":
...
def main() -> int:
    ...
    engine_files, engine_digest = PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.engine_snapshot()
    if args.with_tests and PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.run_tests():
    ...
    print("- next source: Authorial Grammar; C0002 failed substantive review")
    print("- defect: list items separated from semantic lead-in; remaining-source release revoked")
    print("- cumulative spend: $0.200286; active money-only envelope remaining: $1.799714")
    print("- replan, additional provider calls, and full-source release: not authorized")
    print("- machine: one declarative controller; scaffold + complete-source replan; sequential call/review")
    print("- accepted evidence: quantized C0001 pilot only; C0003-C0012 were not called")
