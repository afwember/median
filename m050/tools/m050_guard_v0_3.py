#!/usr/bin/env python3
"""Verify the retired-archive MEDIAN Gate 5 offline foundation."""
...
ACTIVE_INDEX = REPO_ROOT / "m050/extraction/control/M050_Active_Control_Index_v0_8_MEDIANv0_5_0.json"
PRIOR_ACTIVE_INDEX = REPO_ROOT / "m050/extraction/control/M050_Active_Control_Index_v0_7_MEDIANv0_5_0.json"
...
ARCHIVE_RETIREMENT_RECEIPT = REPO_ROOT / "m050/extraction/audit/M050_Extraction_Gate_5_Archive_Retirement_Receipt_v0_1_MEDIANv0_5_0.json"
EVIDENCE_RELOCATION_MANIFEST = REPO_ROOT / "m050/extraction/evidence/legacy/M050_Legacy_Evidence_Relocation_Manifest_v0_1_MEDIANv0_5_0.json"
FROZEN_MANIFEST = REPO_ROOT / "m050/extraction/control/M050_Frozen_Corpus_Manifest_v0_1_MEDIANv0_5_0.json"
...
IGNORED_PARTS = {"__pycache__", ".pytest_cache", "build"}
...
def validate_active_index(errors: list[str]) -> None:
    try:
        ...
        return
    if index.get("execution_state") != "GATE_5_ARCHIVE_RETIRED_LAYER_E_CANDIDATES_PRESERVED":
    ...
    retirement = index.get("archive_retirement_state", {})
    if (
        retirement.get("retired_archive_files") != 9921
        or retirement.get("relocated_evidence_files") != 12
        or retirement.get("relocated_evidence_hash_mismatches") != 0
        or retirement.get("active_archive_directory_allowed") is not False
    ):
        errors.append("active control index has an unexpected archive-retirement state")
...
def validate_layer_e_migration(errors: list[str]) -> tuple[int, int, int]:
    ...
    active_binding = receipt.get("active_control_index", {})
    if (
        active_binding.get("path") != str(PRIOR_ACTIVE_INDEX.relative_to(REPO_ROOT))
        or active_binding.get("sha256") != sha256_file(PRIOR_ACTIVE_INDEX)
    ):
    ...
    verification = receipt.get("verification", {})
    if (
        verification.get("engine_files") != 68
        or verification.get("engine_digest") != "52df2ab229978823d845b18966e6ae27539c479701a541e535370655f68b1395"
        ...
        or verification.get("gate_5_guard_sha256") != sha256_file(REPO_ROOT / "m050/tools/m050_guard_v0_2.py")
    ):
        errors.append("historical Layer E migration verification snapshot mismatch")
...
def validate_archive_retirement(errors: list[str]) -> int:
    if (REPO_ROOT / "m050/archive").exists():
        errors.append("retired m050/archive directory still exists")
    try:
        manifest = json.loads(EVIDENCE_RELOCATION_MANIFEST.read_text(encoding="utf-8"))
        receipt = json.loads(ARCHIVE_RETIREMENT_RECEIPT.read_text(encoding="utf-8"))
        frozen = json.loads(FROZEN_MANIFEST.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        errors.append(f"archive-retirement control cannot be read: {exc}")
        return 0
    if manifest.get("status") != "ACTIVE_ARCHIVE_RETIRED":
        errors.append("legacy-evidence relocation manifest is not active")
    retired_snapshot = manifest.get("retired_archive_snapshot", {})
    frozen_snapshot = frozen.get("archive_snapshot", {})
    if any(
        retired_snapshot.get(key) != frozen_snapshot.get(key)
        for key in ("root", "file_count_excluding_ds_store", "total_bytes", "ordered_path_and_sha256_digest")
    ):
        errors.append("retired archive snapshot does not match the immutable Gate 4 manifest")
    ...
    for item in relocations:
        ...
        if (
            ...
            or not original.startswith("m050/archive/")
            or original in original_paths
        ):
            errors.append(f"invalid or duplicate retired archive path: {original}")
            continue
        if (
            not isinstance(relocated, str)
            or not relocated.startswith("m050/extraction/evidence/legacy/")
            or relocated in relocated_paths
        ):
            errors.append(f"invalid or duplicate relocated evidence path: {relocated}")
            continue
        original_paths.add(original)
        relocated_paths.add(relocated)
        target = REPO_ROOT / relocated
        if not target.is_file() or sha256_file(target) != item.get("sha256"):
            errors.append(f"relocated legacy evidence is missing or changed: {relocated}")
    if len(original_paths) != 12 or len(relocated_paths) != 12:
        errors.append("legacy-evidence relocation coverage must contain exactly 12 files")
    ...
    if (
        bindings.get("relocation_manifest", {}).get("sha256") != sha256_file(EVIDENCE_RELOCATION_MANIFEST)
        or bindings.get("active_control_index", {}).get("sha256") != sha256_file(ACTIVE_INDEX)
    ):
        errors.append("archive-retirement receipt binding mismatch")
    verification = receipt.get("verification", {})
    ...
    if (
        ...
        or verification.get("relocated_evidence_files") != 12
        or verification.get("retired_archive_present") is not False
    ):
        errors.append("archive-retirement verification snapshot mismatch")
...
def main() -> int:
    ...
    relocated_evidence = validate_archive_retirement(errors)
    ...
    print("- retired archive files: 9,921")
