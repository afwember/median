...
def block_record(block, disposition):
    return {
...
def main() -> int:
    ...
    context_blocks = [
        block_record(block, "context_only") for block in selected
        if ledger[block["block_id"]]["disposition"] == "context_only"
    ]
    target_blocks = [
        block_record(block, "eligible") for block in selected
    ...
    if disposition_schema.get("minItems") != len(target_blocks) or disposition_schema.get("maxItems") != len(target_blocks):
        raise SystemExit("pilot schema disposition count does not match target blocks")
    payload = {
