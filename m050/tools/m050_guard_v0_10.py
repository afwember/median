#!/usr/bin/env python3
"""Verify the clean-contract, accepted quantized-pilot extraction-machine checkpoint."""
...
PRIOR_PATH = REPO_ROOT / "m050/tools/m050_guard_v0_9.py"
AGENTS = REPO_ROOT / "AGENTS.md"
AGENTS_OVERRIDE = REPO_ROOT / "AGENTS.override.md"
ACTIVE_INDEX = REPO_ROOT / "m050/extraction/control/M050_Active_Control_Index_v0_15_MEDIANv0_5_0.json"
PREDECESSOR_INDEX = REPO_ROOT / "m050/extraction/control/M050_Active_Control_Index_v0_14_MEDIANv0_5_0.json"
CHECKPOINT = REPO_ROOT / "m050/extraction/control/M050_Current_State_Checkpoint_v0_6_MEDIANv0_5_0.json"
PREDECESSOR_CHECKPOINT = REPO_ROOT / "m050/extraction/control/M050_Current_State_Checkpoint_v0_5_MEDIANv0_5_0.json"
CHECKPOINT_REPORT = REPO_ROOT / "m050/extraction/control/M050_Current_State_Checkpoint_v0_6_MEDIANv0_5_0.md"
BOOTSTRAP = REPO_ROOT / "m050/extraction/control/M050_New_Task_Bootstrap_v0_5_MEDIANv0_5_0.md"
STANDARD = REPO_ROOT / "m050/extraction/control/M050_Compile_Execution_Standard_v0_2_MEDIANv0_5_0.md"
CONFIG = REPO_ROOT / "m050/extraction/control/M050_Authorial_Grammar_Extraction_Machine_Config_v0_2_MEDIANv0_5_0.json"
...
COMPATIBILITY = REPO_ROOT / "m050/extraction/audit/M050_Authorial_Grammar_Quantized_Machine_Compatibility_Receipt_v0_2_MEDIANv0_5_0.json"
PLAN = REPO_ROOT / "m050/extraction/control/M050_Authorial_Grammar_Section_Aware_Chunk_Plan_v0_2_MEDIANv0_5_0.json"
FREEZE = REPO_ROOT / "m050/extraction/calibration/authorial-grammar/M050_Authorial_Grammar_Quantized_C0001_Pilot_Freeze_Proposal_v0_7_MEDIANv0_5_0.json"
HALT = REPO_ROOT / "m050/extraction/audit/source-run-transitions/M050_Authorial_Grammar_Full_Source_Run_Halted_After_C0001_v0_1_MEDIANv0_5_0.json"
SPEND = REPO_ROOT / "m050/extraction/audit/spend-envelopes/M050_Provider_Spend_Envelope_001_After_Quantized_C0001_v0_3_MEDIANv0_5_0.json"
...
REVIEW_COMPATIBILITY = REPO_ROOT / "m050/extraction/audit/M050_Authorial_Grammar_Quantized_C0001_Review_Gate_Compatibility_Receipt_v0_1_MEDIANv0_5_0.json"
PILOT_LEDGER = REPO_ROOT / "m050/extraction/runs/authorial-grammar-quantized-pilot/M050_Authorial_Grammar_Quantized_Pilot_Run_Ledger_v0_7_MEDIANv0_5_0.jsonl"
...
def validate_root_contract(errors: list[str]) -> None:
    ...
    required = (
        "M050_Active_Control_Index_v0_15_MEDIANv0_5_0.json",
        "M050_Current_State_Checkpoint_v0_6_MEDIANv0_5_0.md",
        "M050_New_Task_Bootstrap_v0_5_MEDIANv0_5_0.md",
        "m050_guard_v0_10.py",
        "24 / 22 / 4 / 18 = 14 + 4",
        "cumulative spend envelope",
        "zero-call `scaffold`",
        "Chunk count is a generated result",
        "target blocks per chunk",
    )
...
def validate_active_index(errors: list[str]) -> dict:
    ...
    if index.get("schema_version") != "M050-ACTIVE-CONTROL-INDEX-0.15":
    ...
    if index.get("execution_state") != "AUTHORIAL_GRAMMAR_QUANTIZED_C0001_PILOT_ACCEPTED_WAITING_FULL_SOURCE_RELEASE":
    ...
    if (
        ...
        or calibration.get("current_state") != "quantized_pilot_accepted_waiting_full_source_release"
        or calibration.get("pilot_acceptance_releases_full_source") is not False
        or calibration.get("original_source_run_revoked") is not True
        or calibration.get("target_blocks_per_chunk") != 20
        or calibration.get("generated_chunk_count") != 12
        or calibration.get("chunk_count_is_input") is not False
    ...
    required_machine = {
        ...
        "complete_source_replan_after_calibration_required": True,
        "manifest_chunk_count_permitted": False,
        "claude_cache_ttl": "1h",
        ...
        "cumulative_spent_usd": "0.137431",
        "remaining_usd": "1.862569",
...
def validate_machine(errors: list[str]) -> None:
    ...
    if (
        config.get("status") != "OFFLINE_RECALIBRATION_REQUIRES_PILOT"
        or quantization.get("target_blocks_per_chunk") != 20
        or quantization.get("generated_chunk_count") != 12
        or quantization.get("chunk_count_is_input") is not False
    ):
        errors.append("Authorial machine quantization binding drifted")
    for path in (STANDARD, CONTROLLER, ENGINE_MODULE, ENGINE_TEST, COMPATIBILITY, PLAN, FREEZE, HALT, SPEND, ACCEPTED_PILOT, REVIEW_COMPATIBILITY, PILOT_LEDGER):
        if not path.is_file():
            errors.append(f"missing extraction-machine control: {path.relative_to(REPO_ROOT)}")
    plan = read_json(PLAN, errors)
    chunks = plan.get("chunks", [])
    targets = [chunk.get("target_blocks") for chunk in chunks]
    if (
        ...
        or targets != [20] * 10 + [17, 11]
    ):
        errors.append("Authorial complete-source requantization drifted")
    compatibility = read_json(COMPATIBILITY, errors)
    if (
        compatibility.get("status") != "OFFLINE_REQUANTIZATION_VERIFIED_PILOT_REQUIRED"
        or compatibility.get("offline_verification", {}).get("offline_tests_passed") != 116
        or compatibility.get("authority_boundary", {}).get("provider_calls_authorized") != 0
    ):
        errors.append("quantized-machine compatibility boundary drifted")
    accepted = read_json(ACCEPTED_PILOT, errors)
    if (
        accepted.get("state") != "pilot_accepted"
        or accepted.get("pilot_result") != "perfect_for_release_for_20_target_block_quantization"
        or accepted.get("additional_provider_calls_authorized") != 0
        or accepted.get("full_source_execution_authorized") is not False
    ):
        errors.append("quantized C0001 pilot acceptance drifted")
    review_compatibility = read_json(REVIEW_COMPATIBILITY, errors)
    if (
        review_compatibility.get("status") != "OFFLINE_REVIEW_GATE_REPAIR_VERIFIED"
        or review_compatibility.get("verification", {}).get("complete_tests_passed") != EXPECTED_TESTS
        or review_compatibility.get("authority_boundary", {}).get("additional_provider_calls_authorized") != 0
    ):
        errors.append("review-gate compatibility boundary drifted")
    ledger_lines = PILOT_LEDGER.read_text(encoding="utf-8").splitlines() if PILOT_LEDGER.is_file() else []
    try:
        ledger_states = [json.loads(line).get("state") for line in ledger_lines if line.strip()]
    except json.JSONDecodeError:
        ledger_states = []
    if ledger_states != ["call_captured", "review_passed"]:
        errors.append("quantized pilot ledger state drifted")
    halt = read_json(HALT, errors)
    if halt.get("release_revoked") is not True or halt.get("additional_provider_calls_authorized") != 0:
        errors.append("halted source-run boundary drifted")
    spend = read_json(SPEND, errors)
    if (
        spend.get("active") is not True
        or spend.get("spent_usd") != "0.137431"
        or spend.get("remaining_usd") != "1.862569"
    ):
        errors.append("reconciled spend-envelope boundary drifted")
...
def validate_checkpoint(errors: list[str]) -> None:
    ...
    if checkpoint.get("status") != "AUTHORIAL_GRAMMAR_QUANTIZED_C0001_PILOT_ACCEPTED_WAITING_FULL_SOURCE_RELEASE":
...
def main() -> int:
    ...
    engine_files, engine_digest = PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.engine_snapshot()
    if args.with_tests and PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.PRIOR.run_tests():
    ...
    print("- next source: Authorial Grammar; quantized C0001 pilot accepted")
    print("- cumulative spend: $0.137431; active money-only envelope remaining: $1.862569")
    print("- additional provider calls and full-source release: not authorized")
    print("- machine: one declarative controller; scaffold + complete-source replan; sequential call/review")
    print("- quantization: 20 target blocks; 12 generated chunks; C0002-C0012 remain")
    print("- review gate: decision-required findings need explicit review; mechanical failures remain blocked")
